...
def _gym_store(owner: Optional[str]) -> tuple[bool, List[Dict[str, Any]]]:
    ...
    for row in value["entries"]:
        ...
        entries.append({
            ...
            "date": workout_date,
            "title": title,
            "duration": _clean_text(row.get("duration") or row.get("total_time"), 30),
            "work_time": _clean_text(row.get("work_time"), 30),
            "rest_time": _clean_text(row.get("rest_time"), 30),
            "total_sets": row.get("total_sets"),
            "total_reps": row.get("total_reps"),
            "resting_calories": row.get("resting_calories"),
            ...
            "avg_time_per_set": _clean_text(row.get("avg_time_per_set"), 30),
            ...
            "intensity_minutes_total": row.get("intensity_minutes_total"),
            "body_battery_net_impact": row.get("body_battery_net_impact"),
            "primary_benefit": _clean_text(row.get("primary_benefit"), 160),
            "muscle_primary": _clean_text(row.get("muscle_primary"), 300),
            "muscle_secondary": _clean_text(row.get("muscle_secondary"), 300),
    ...
    def _gym_candidates(
        ...
        for entry in entries[:8]:
            ...
            for key, label in (
                ...
                ("work_time", "Work time"), ("rest_time", "Rest time"),
                ("resting_calories", "Resting calories"),
                ("total_calories", "Total calories"),
                ("estimated_sweat_loss_ml", "Estimated sweat loss ml"),
                ("avg_time_per_set", "Average time per set"),
                ("total_volume", "Total volume"),
                ("intensity_minutes_moderate", "Moderate intensity minutes"),
                ("intensity_minutes_vigorous", "Vigorous intensity minutes"),
                ("intensity_minutes_total", "Total intensity minutes"),
                ("body_battery_net_impact", "Body Battery net impact"),
                ("primary_benefit", "Primary benefit"),
                ("muscle_primary", "Primary muscles"),
                ("muscle_secondary", "Secondary muscles"),
            ):
