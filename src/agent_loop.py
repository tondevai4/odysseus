...
- Notes follow-ups remain Notes actions. When the assistant has formatted a pending note draft and the user says "create then", "create it", "go ahead", "yes", "do it", "save it", "add it", or "make the note", recover the title and formatted content from recent conversation history and call `manage_notes` immediately. Do not claim Notes are unavailable when the tool is present. "Add to notes: <content>" requires only a missing title; do not ask which note system. For "new note gym log with date please", infer a title such as "Gym Log — 15 Jun 2026" from the current date. Ask at most one formatting question, and retain the formatted draft for the next confirmation.
...
- Calendar: call `manage_calendar` with `action=list_calendars` FIRST before create/update/delete operations.
- "Create/add/write a note" / "notes" / "todos" / "remind me to X at <time>" → use `manage_notes`. Do NOT store notes in `manage_memory`; memory is for persistent facts/preferences about the user, not note content. For reminders, include a `due_date`; for todos, use `note_type=checklist` when appropriate. To add text or checklist items to a clearly named existing note, use action=append with its exact title. If add reports duplicate/requires_user_choice, stop and ask whether to append, rename, or cancel; never append automatically. Never delete, archive, replace, rename, or overwrite a note from chat. After a successful add/append, reply exactly: "Done, Boss. Saved to Notes." `manage_tasks` is for RECURRING background AI jobs, NOT for one-off user reminders.
...
```
Pending Notes follow-ups must use this tool. When a formatted draft is already in recent conversation history and the user says "create then", "create it", "go ahead", "yes", "do it", "save it", "add it", or "make the note", recover the pending title/content and call `add` immediately with `note_type="note"`. Do not claim Notes are unavailable while this tool is present. "Add to notes: <content>" asks only for a missing title, never which note system. For "new note gym log with date please", infer a current-date title such as "Gym Log — 15 Jun 2026"; if formatting is requested, retain that formatted draft for the next confirmation.
Notes, checklists, AND user reminders. Use this for "create/add/write a note", todos, checklists, and "remind me to X at <time>" — never use memory for note content. For reminders, pair a short `title` (what to do) with a `due_date` (when). `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours", "next monday 9am") or ISO ("2026-05-12T13:00:00"). Chat actions: `list`, `add` (title, content OR items:[{text,done}], note_type, color, label, due_date), `append` (exact title plus content or checklist_items), `toggle_item`. If add reports duplicate/requires_user_choice, stop and ask whether to append, rename, or cancel; never append automatically. Never delete, archive, replace, rename, or overwrite notes from chat. After add/append succeeds, reply exactly: "Done, Boss. Saved to Notes." """,
...
_EXPLICIT_CONTINUATION_RE = re.compile(
    r"^\s*(?:"
    r"yes|y|yeah|yep|ok|okay|sure|create then|create it|save it|add it|"
    r"make the note|do it|go ahead|continue|carry on|"
